      ...
      <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
        <form class="card space-y-5 p-5 sm:p-6" @submit.prevent="submit">
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label class="input-label" for="channel-onboarding-concurrency">
                {{ t('admin.channelOnboarding.form.concurrency') }} <span class="text-red-500">*</span>
              </label>
              <input
                id="channel-onboarding-concurrency"
              ...
              />
              <p class="input-hint">{{ t('admin.channelOnboarding.form.concurrencyHint') }}</p>
            </div>
...
const platformOptions = computed(() =>
  CHANNEL_ONBOARDING_PLATFORMS.map(value => ({
    value,
