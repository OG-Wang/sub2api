export default {
    settings: {
      ...
      features: {
        channelMonitor: {
          title: '渠道监控',
          description: '启用后可选择 V1 主动探测、V2 被动用量监控，或两者并存。关闭后两种模式的后台任务均停止，用户端入口隐藏。',
          configureLink: '前往 渠道管理 > 渠道监控 配置监控项',
          enabled: '启用渠道监控',
          enabledHint: '关闭后 V1 调度器与 V2 聚合均停止；已有配置与历史保留。',
          mode: '监控模式',
          modeHint: '默认 V1（主动探测）。需要被动聚合时切换到 V2；供应商大厅需要两套数据，选「并存」。',
          modeV2: 'V2 被动监控',
          modeV1: 'V1 主动探测',
          modeHybrid: 'V1 + V2 并存',
          modeV2Hint: '需主动选择：基于真实网关流量聚合健康指标，不向上游发送探活请求；启用期间 V1 探测停止。',
          modeV1Hint: '默认模式：按配置的渠道监控项定时发起上游健康检查（产生探测流量）。',
          modeHybridHint: '主动探测与被动聚合同时运行，供应商大厅页需要此模式。注意：两套后台任务负载相加，建议先在低峰期观察一轮。',
