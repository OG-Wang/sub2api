export default {
    ...
    channelMonitor: {
      ...
      providerHall: {
        section: '渠道状态页展示（可选）',
        sectionHint: '控制该监控项是否出现在用户端「渠道状态」页面，以及关联到哪个真实分组。仅在监控模式为「V1 + V2 并存」时生效。',
        publicVisible: '在渠道状态页展示',
        publicVisibleHint: '默认关闭。开启后所有登录用户都能看到该渠道的延迟、可用率与缓存命中率。',
        group: '关联分组',
        groupPlaceholder: '选择分组（可留空）',
        groupHint: '关联后才能显示分组倍率、拼接用户实际用量指标，并让「使用此分组」按钮可点。留空则只按分组名文本展示。',
        groupMissing: '当前关联的分组（ID {id}）已不存在，保存时请重新选择或清空。',
        publicNote: '分组备注',
        publicNotePlaceholder: '如：该渠道频繁调整倍率，请注意设置最高倍率限制',
        publicNoteHint: '展示在渠道状态页的分组名下方，最多 200 字。',
        reportUrl: '检测报告链接',
        reportUrlHint: '可选的第三方检测报告外链，留空则不显示该链接。仅支持 http/https。',
        reportUrlInvalid: '链接无效，必须是 http:// 或 https:// 开头的完整地址。',
        expectedInputTokens: '期望输入 Token',
        expectedInputTokensPlaceholder: '留空 = 自动学习',
        expectedInputTokensHint:
          '探测请求的输入 token 基线。探测 prompt 长度恒定，实测值明显高于基线说明上游注入了额外内容（如私自加 system prompt）。留空则由系统按历史数据自动学习。',
      },
      advanced: {
        section: '高级（可选）',
    ...
    channelOnboarding: {
      ...
      form: {
        ...
        apiKeyHint: '密钥仅用于创建账号并加密保存，不会在成功结果中返回。',
        primaryModel: '主模型',
      ...
      autoConfig: {
        ...
        monitor: '创建同名监控：使用填写的监测间隔、公开展示、使用当前服务地址。'
      },
      submit: '开始接入',
      submitting: '接入中...',
      error: '渠道接入失败',
