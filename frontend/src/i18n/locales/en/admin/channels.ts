export default {
    ...
    channelMonitor: {
      ...
      providerHall: {
        section: 'Channel Status page (optional)',
        sectionHint: 'Controls whether this monitor appears on the user-facing Channel Status page, and which real group it links to. Only takes effect when the monitor mode is "V1 + V2 hybrid".',
        publicVisible: 'Show on Channel Status',
        publicVisibleHint: 'Off by default. When on, every logged-in user can see this channel\'s latency, availability and cache hit rate.',
        group: 'Linked group',
        groupPlaceholder: 'Select a group (optional)',
        groupHint: 'Linking a group enables the rate multiplier, real user metrics, and the "Use this group" button. Without it only the group name text is shown.',
        groupMissing: 'The linked group (ID {id}) no longer exists. Pick another one or clear it before saving.',
        publicNote: 'Group note',
        publicNotePlaceholder: 'e.g. This channel changes its rate often — set a max rate limit',
        publicNoteHint: 'Rendered under the group name on the Channel Status page. Up to 200 characters.',
        reportUrl: 'Detection report link',
        reportUrlHint: 'Optional third-party report link. Leave empty to hide it. Only http/https is allowed.',
        reportUrlInvalid: 'Invalid link — must be a full http:// or https:// URL.',
        expectedInputTokens: 'Expected input tokens',
        expectedInputTokensPlaceholder: 'Empty = auto-learn',
        expectedInputTokensHint:
          'Baseline input token count for probe requests. The probe prompt has a constant length, so a measured value well above the baseline means the upstream injected extra content (e.g. its own system prompt). Leave empty to auto-learn from history.',
      },
      advanced: {
    ...
    channelOnboarding: {
      ...
      form: {
        ...
        apiKeyHint: 'The key is encrypted for account storage and is never returned in the success result.',
        primaryModel: 'Primary model',
      ...
      autoConfig: {
        ...
        monitor: 'Creates a same-name monitor with the selected interval, public visibility, and the current service URL.'
      },
