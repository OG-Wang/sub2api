...
func TestValidateChannelOnboardingRequest(t *testing.T) {
	tests := []struct {
		...
		{name: "zero concurrency", mutate: func(r *ChannelOnboardingRequest) { n := 0; r.Concurrency = &n }},
		{name: "negative concurrency", mutate: func(r *ChannelOnboardingRequest) { n := -1; r.Concurrency = &n }},
	}
...
func TestNewChannelOnboardingPlanNormalizes(t *testing.T) {
	...
	require.Equal(t, 25, plan.Concurrency)
	...
	req.Concurrency = nil
	fallback := newChannelOnboardingPlan(req, "")
	require.Equal(t, defaultChannelOnboardingIntervalSeconds, fallback.IntervalSeconds)
	require.Equal(t, defaultOnboardingAccountConcurrency, fallback.Concurrency)
}
...
func TestChannelOnboardingPlanBuildAccountEnablesProbeAndRateSync(t *testing.T) {
	for _, platform := range ChannelOnboardingPlatforms() {
		t.Run(platform, func(t *testing.T) {
			...
			require.Equal(t, defaultOnboardingAccountConcurrency, account.Concurrency)
...
func TestChannelOnboardingPlanResultNeverExposesPlaintextKey(t *testing.T) {
	...
	require.Equal(t, defaultOnboardingAccountConcurrency, result.Concurrency)
