...
const (
	...
	// defaultOnboardingAccountConcurrency matches the account create form's
	// default; the form there accepts any integer >= 1, so onboarding does not
	// invent an upper bound of its own.
	defaultOnboardingAccountConcurrency = 10
...
func newChannelOnboardingPlan(req ChannelOnboardingRequest, endpoint string) channelOnboardingPlan {
	...
	concurrency := defaultOnboardingAccountConcurrency
...
func validateChannelOnboardingRequest(req ChannelOnboardingRequest) error {
	...
	// Mirrors the account create form: any integer >= 1. No upper bound,
	// because upstream's account create path does not impose one either.
	if req.Concurrency != nil && *req.Concurrency < 1 {
		return infraerrors.BadRequest("CHANNEL_ONBOARDING_CONCURRENCY_INVALID", "concurrency must be at least 1")
	}
	if req.IntervalSeconds != nil {
